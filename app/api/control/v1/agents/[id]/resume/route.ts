// POST /api/control/v1/agents/{id}/resume — lift a per-agent suspension (write scope).
...
const handler = control("write", async ({ userId, db, params, keyId, requestId }) => {
  ...
  const r = await setAgentSuspended(db, userId, id, false);
  ...
  await recordAdminAction({
    ...
    targetType: "agent",
    targetId: id,
    metadata: { suspended: false, via: "api", key_id: keyId },
  });
  return jsonResponse({ data: { id, status: "active" } }, requestId);
});
