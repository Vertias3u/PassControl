...
// This card paints an SVG that is exported as an image, so every colour has to be
// a real value — a CSS custom property does not survive the export. That is why
// the accent is duplicated here rather than read from var(--pc-brand).
//
// The neutral surfaces carry a trace of the accent; without it the export looks
// like a stock template. So each accent gets its own set, generated by holding
// every role's saturation and lightness exactly and moving only the hue. Cloud's
// green set is the original, preserved byte-for-byte rather than regenerated, so
// the hosted card cannot shift by a single value.
const NEUTRALS = {
  sky: {
    ink: "#0b0e0f",
    panel: "#111517",
...
/** Semantic states never follow the accent — a warning is amber in every theme. */
function paletteFor(accent: PassportAccent = DEFAULT_ACCENT) {
  return {
    ...NEUTRALS[accent],
...
function lifecycleColor(status: string, accent: PassportAccent): string {
...
function PassportSvg({
  ...
  return (
    ...
    >
      ...
      <circle cx="394" cy="363" r="7" fill={lifecycleColor(passport.agent.status, accent)} />
      <text
        x="412"
        y="370"
        fill={lifecycleColor(passport.agent.status, accent)}
        fontFamily="ui-monospace, SFMono-Regular, Menlo, monospace"
        fontSize="22"
        fontWeight="800"
        letterSpacing="1.2"
...
export function AgentPassport({
  ...
  // Which accent the exported card is painted in. Server-supplied for the same
  // reason as the TTL: the choice lives in a cookie this component cannot read,
  // and reading it after mount would repaint the card in front of the operator.
  // Omitted on a deployment with one accent, which is every self-hosted one.
  accent?: PassportAccent;
}) {
  const { format } = useDashboardTime();
  ...
  return (
    <>
      ...
      <section className="grid gap-4" aria-labelledby="agent-passport-heading">
        ...
        >
          <PassportSvg passport={passport} showFullId={showFullId} svgRef={svgRef} accent={accent} />
        </div>
